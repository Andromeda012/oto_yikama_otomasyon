<template>
  <section class="market-page">
    ...
    <section class="panel recent-panel">
      <div class="panel-head"><div><h2>Son stok hareketleri</h2><span>Market satışları ve hizmet tüketimleri</span></div></div>
      <div v-if="!movements.length" class="empty compact">Henüz stok hareketi bulunmuyor.</div>
      <div v-else class="sales-table-wrap"><table><thead><tr><th>Tarih</th><th>Ürün</th><th>Hareket</th><th>Önce</th><th>Sonra</th><th class="right">Miktar</th></tr></thead><tbody><tr v-for="item in movements" :key="item.id"><td>{{ formatDate(item.created_at) }}</td><td><strong>{{ item.product_name }}</strong></td><td><span class="movement-badge" :class="item.movement_type">{{ movementLabel(item.movement_type) }}</span></td><td>{{ number(item.stock_before) }} {{ item.unit }}</td><td>{{ number(item.stock_after) }} {{ item.unit }}</td><td class="right">{{ number(item.quantity) }} {{ item.unit }}</td></tr></tbody></table></div>
    </section>
  </section>
</template>
...
import { createMarketSale, getMarketLookups, getMarketProducts, getMarketSummary, getRecentSales, getStockMovements } from "../services/market";

const products = ref([]), sales = ref([]), movements = ref([]), search = ref(""), loading = ref(false), productsLoading = ref(false), saving = ref(false), error = ref(""), success = ref("");
...
async function loadMovements() { movements.value = await getStockMovements(50); }
async function loadLookups() { Object.assign(lookups, await getMarketLookups()); }
async function refreshAll() { loading.value = true; error.value = ""; try { await Promise.all([loadProducts(), loadSummary(), loadSales(), loadMovements(), loadLookups()]); } catch (e) { error.value = e.message; } finally { loading.value = false; } }
...
function movementLabel(type) { return ({ sale: "Market satışı", service_consumption: "Hizmet tüketimi", purchase: "Stok girişi", adjustment: "Düzeltme" })[type] || type; }
